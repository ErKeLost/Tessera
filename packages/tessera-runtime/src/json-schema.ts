...
import { jsonValueSchema, type JsonObject, type JsonValue, type StateDefinition } from "./schemas";

export type JsonSchema = boolean | JsonObject;

export type JsonSchemaProfileLimits = {
...
export const DEFAULT_JSON_SCHEMA_PROFILE_LIMITS: Readonly<JsonSchemaProfileLimits> = Object.freeze({
  maxSchemaDepth: 32,
...
export class JsonSchemaContractError extends Error {
  readonly code: string;

  constructor(code: string, message: string) {
    super(message);
    this.name = "JsonSchemaContractError";
    this.code = code;
  }
}
...
export async function prepareJsonSchema(
  schema: JsonSchema,
  expectedHash: string,
  options: { limits?: Partial<JsonSchemaProfileLimits> } = {},
): Promise<PreparedJsonSchema> {
  const limits = { ...DEFAULT_JSON_SCHEMA_PROFILE_LIMITS, ...options.limits };
  assertBoundedJsonSchema(schema, limits);
  const schemaHash = await canonicalHash(schema);
  if (schemaHash !== expectedHash) {
    throw new JsonSchemaContractError("schema.hash-mismatch", "JSON Schema hash does not match its declared identity.");
  }
  ...
  try {
    ...
    throw new JsonSchemaContractError(
      "schema.compile-failed",
      `JSON Schema cannot be compiled by the runtime profile: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return { schema: structuredClone(schema), schemaHash, validator };
}

export async function prepareStateSchema(
  definition: Pick<StateDefinition, "schema" | "schemaHash">,
  options: { limits?: Partial<JsonSchemaProfileLimits> } = {},
): Promise<PreparedJsonSchema> {
  return prepareJsonSchema(definition.schema, definition.schemaHash, options);
}
...
export function parseJsonWithSchema(validator: z.ZodType, input: unknown): JsonValue {
  ...
  if (!json.success) {
    throw new JsonSchemaContractError("schema.non-json-value", "Value is not finite, prototype-safe JSON.");
  }
  const result = validator.safeParse(json.data);
  if (!result.success) {
    throw new JsonSchemaContractError("schema.value-invalid", result.error.issues.map((issue) => issue.message).join("; "));
  }
  const output = jsonValueSchema.safeParse(result.data);
  if (!output.success) {
    throw new JsonSchemaContractError("schema.output-non-json", "Schema transformation produced a non-JSON value.");
  }
  return output.data;
}
...
export function assertBoundedJsonSchema(
  schema: JsonSchema,
  limits: JsonSchemaProfileLimits = DEFAULT_JSON_SCHEMA_PROFILE_LIMITS,
): void {
  if (schema === true) throw new JsonSchemaContractError("schema.unbounded", "The true JSON Schema is not allowed.");
  ...
  const visit = (candidate: unknown, depth: number, path: string, refStack: readonly string[]): void => {
    nodes += 1;
    if (nodes > limits.maxSchemaNodes) throw new JsonSchemaContractError("schema.node-limit", "JSON Schema is too large.");
    if (depth > limits.maxSchemaDepth) throw new JsonSchemaContractError("schema.depth-limit", "JSON Schema is too deeply nested.");
    if (candidate === false) return;
    if (candidate === true || candidate === null || typeof candidate !== "object" || Array.isArray(candidate)) {
      throw new JsonSchemaContractError("schema.invalid-node", `Invalid or unbounded schema node at ${path}.`);
    }
    const node = candidate as Record<string, unknown>;
    for (const key of Object.keys(node)) {
      if (!ALLOWED_KEYS.has(key)) {
        throw new JsonSchemaContractError("schema.unsupported-keyword", `Unsupported JSON Schema keyword ${key} at ${path}.`);
      }
    }
    if (typeof node.$schema === "string" && !KNOWN_DIALECTS.has(node.$schema)) {
      throw new JsonSchemaContractError("schema.unsupported-dialect", `Unsupported JSON Schema dialect at ${path}.`);
    }
    if (node.$ref !== undefined) {
      if (typeof node.$ref !== "string" || !node.$ref.startsWith("#/")) {
        throw new JsonSchemaContractError("schema.remote-ref", `Only local JSON Pointer references are allowed at ${path}.`);
      }
      const siblings = Object.keys(node).filter((key) => key !== "$ref" && key !== "$comment" && key !== "description");
      if (siblings.length > 0) throw new JsonSchemaContractError("schema.ref-siblings", `$ref siblings are not allowed at ${path}.`);
      if (refStack.includes(node.$ref)) throw new JsonSchemaContractError("schema.recursive-ref", `Recursive schema reference at ${path}.`);
    ...
    if (variants.length > 1) throw new JsonSchemaContractError("schema.ambiguous-composition", `Only one composition keyword is allowed at ${path}.`);
    for (const key of ["anyOf", "oneOf", "allOf"] as const) {
      const branches = node[key];
      if (branches === undefined) continue;
      if (!Array.isArray(branches) || branches.length === 0 || branches.length > limits.maxUnionBranches) {
        throw new JsonSchemaContractError("schema.union-limit", `Invalid ${key} branch count at ${path}.`);
      }
    ...
    if (node.enum !== undefined) {
      if (!Array.isArray(node.enum) || node.enum.length === 0 || node.enum.length > limits.maxUnionBranches) {
        throw new JsonSchemaContractError("schema.enum-limit", `Invalid enum at ${path}.`);
      }
      for (const value of node.enum) {
        if (!jsonValueSchema.safeParse(value).success) {
          throw new JsonSchemaContractError("schema.invalid-enum", `Non-JSON enum at ${path}.`);
        }
      }
    }
    ...
    if (!hasShape) throw new JsonSchemaContractError("schema.unbounded", `Schema has no constraining type or composition at ${path}.`);
    if (Array.isArray(type)) throw new JsonSchemaContractError("schema.type-array", `Type arrays are not in the runtime profile at ${path}.`);
    if (type !== undefined && !["null", "boolean", "number", "integer", "string", "array", "object"].includes(String(type))) {
      throw new JsonSchemaContractError("schema.unsupported-type", `Unsupported type at ${path}.`);
    }
    if (typeof node.format === "string" && !ALLOWED_FORMATS.has(node.format)) {
      throw new JsonSchemaContractError("schema.unsupported-format", `Unsupported string format ${node.format} at ${path}.`);
    }
    ...
    if (type === "array") {
      ...
      if (node.items === undefined || Array.isArray(node.items)) {
        throw new JsonSchemaContractError("schema.unbounded-items", `Arrays require one bounded items schema at ${path}.`);
      }
    ...
    if (type === "object") {
      ...
      if (properties !== undefined && (properties === null || typeof properties !== "object" || Array.isArray(properties))) {
        throw new JsonSchemaContractError("schema.invalid-properties", `Invalid properties map at ${path}.`);
      }
      const propertyEntries = Object.entries((properties ?? {}) as Record<string, unknown>);
      if (propertyEntries.length > limits.maxObjectProperties) {
        throw new JsonSchemaContractError("schema.property-limit", `Too many properties at ${path}.`);
      }
      if (node.required !== undefined) {
        if (!Array.isArray(node.required) || node.required.some((key) => typeof key !== "string")) {
          throw new JsonSchemaContractError("schema.invalid-required", `required must be a string array at ${path}.`);
        }
        if (new Set(node.required).size !== node.required.length) {
          throw new JsonSchemaContractError("schema.duplicate-required", `required contains duplicates at ${path}.`);
        }
      }
      ...
      ) throw new JsonSchemaContractError("schema.property-range", `minProperties exceeds maxProperties at ${path}.`);
      ...
      if (node.additionalProperties !== false) {
        ...
        if (node.additionalProperties === undefined || node.additionalProperties === true) {
          throw new JsonSchemaContractError("schema.untyped-properties", `Open objects require a typed additionalProperties schema at ${path}.`);
        }
  ...
  for (const refsKey of ["$defs", "definitions"] as const) {
    ...
    if (defs === null || typeof defs !== "object" || Array.isArray(defs)) {
      throw new JsonSchemaContractError("schema.invalid-definitions", `Invalid ${refsKey}.`);
    }
...
function resolveLocalRef(root: Record<string, unknown>, ref: string): unknown {
  ...
  for (const raw of ref.slice(2).split("/")) {
    ...
    if (current === null || typeof current !== "object" || Array.isArray(current) || !Object.hasOwn(current, segment)) {
      throw new JsonSchemaContractError("schema.missing-ref", `Local reference does not exist: ${ref}.`);
    }
...
function assertBound(value: unknown, ceiling: number, keyword: string, path: string): void {
  if (!Number.isInteger(value) || (value as number) < 0 || (value as number) > ceiling) {
    throw new JsonSchemaContractError("schema.unbounded", `${keyword} must be an integer no greater than ${ceiling} at ${path}.`);
  }
}
