...
function resolveStudioEntry(options = {}) {
  ...
  for (const specifier of [
    "@open-tessera/studio/main",
    "@open-tessera/studio/server",
  ]) {
    try {
      return resolvePackage(specifier);
    } catch {}
  }
  throw new Error("Could not find the Tessera Studio server. Reinstall data-elements so its @open-tessera/studio dependency is present, or run this command from a Tessera source checkout.");
}
