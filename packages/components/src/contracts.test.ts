...
describe("official component catalog", () => {
  test("creates exactly 12 component contracts and three action contracts at package revision 0.3.11", async () => {
    const catalog = await createOfficialCatalog();
    const packageJson = await Bun.file(new URL("../package.json", import.meta.url)).json();

    expect(packageJson.version).toBe("0.3.11");
    expect(String(OFFICIAL_CATALOG_REVISION)).toBe("0.3.11");
