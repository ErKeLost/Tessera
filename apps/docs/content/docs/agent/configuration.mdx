...
The current product profile is deliberately fixed to 12 orthogonal Component
Contracts and the complete supported shadcn/Recharts chart recipe set. Product
shapes such as trend, funnel, cohort, anomaly, and forecast are recipes over the
same contracts; they are not extra protocol component types.
