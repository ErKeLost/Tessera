...
> Tessera Agent/Studio monorepo release process as design history. Current
> repository verification starts from the root commands in
> [`README.md`](../README.md).
...
for its then-current package scopes. Never put a registry token in the
repository or local package manifests.
...
endpoint are intentionally not reproduced as runnable commands because this
page is historical.
